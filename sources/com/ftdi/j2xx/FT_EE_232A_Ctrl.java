...
public class FT_EE_232A_Ctrl extends FT_EE_Ctrl {
    ...
    public FT_EE_232A_Ctrl(FT_Device usbC) {
        super(usbC);
    }
    ...
    public short programEeprom(FT_EEPROM ee) {
        ...
        try {
            ...
            int saddr2 = setStringDescriptor(ee.Product, data, saddr + ee.Manufacturer.length() + 2, 8, true) + ee.Product.length() + 2;
            if (ee.SerNumEnable) {
                int stringDescriptor = setStringDescriptor(ee.SerialNumber, data, saddr2, 9, true) + ee.SerialNumber.length() + 2;
            }
            int saddr3 = data[1];
            if (saddr3 == 0 || data[2] == 0) {
                return (short) 2;
            }
